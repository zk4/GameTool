...
{
    static  vector<OrthoNode*> updated_nodes;
    updated_nodes.clear();
    for (auto choosen_node : choosen_nodes_)
    {
        ...
        while (edge)
        {
            ...
            updated_nodes.push_back (toNode);
            edge = edge->nextOutedge;
        }
    }

    //find smallest
    OrthoNode* smallestnode = NULL;

    if (updated_nodes.size() > 0)
    {
        smallestnode = *updated_nodes.begin();
    }

    for (auto un : updated_nodes)
    {
        if (smallestnode->_distance_from_start > un->_distance_from_start)
        {
            smallestnode = un;
        }
    }
